<?php
$cache_file = __DIR__ . "/cache/". str_replace(":", "-", $ip) .".json";

//Ban System
$querybanned = $mysqli->query("SELECT ip FROM `bansystem_bans` WHERE ip='$ip' LIMIT 1");
if ($querybanned->num_rows > 0) {
    $bannedpage_url = $settings['bansystem_path'] . "/pages/banned.php";
    echo '<meta http-equiv="refresh" content="0;url=' . $bannedpage_url . '" />';
    exit;
}

//IP Ranges
$querybanned = $mysqli->query("SELECT ip_range FROM `bansystem_bans-ranges` WHERE ip_range='$ip_range' LIMIT 1");
if ($querybanned->num_rows > 0) {
    $bannedpage_url = $settings['bansystem_path'] . "/pages/banned.php";
    echo '<meta http-equiv="refresh" content="0;url=' . $bannedpage_url . '" />';
    exit;
...
//Blocking Country
$query1 = $mysqli->query("SELECT * FROM `bansystem_bans-country`");
$query2 = $mysqli->query("SELECT * FROM `bansystem_bans-other` WHERE type = 'isp'");
if ($query1->num_rows > 0 OR $query2->num_rows > 0) {
	if (bansys_getcache($cache_file) == 'BANSYS_NoCache') {
		...
		$ip_data = @json_decode(bansys_getcache($cache_file));
	}
...
@$querybanned = $mysqli->query("SELECT id, country FROM `bansystem_bans-country` WHERE country='$country_check'");
@$rowcb = mysqli_fetch_array($querybanned);

if ($settings['countryban_blacklist'] == 1) {
    if ($querybanned->num_rows > 0) {
        $bannedcpage_url = $settings['bansystem_path'] . "/pages/banned-country.php?c_id=" . $rowcb['id'];
    ...
    if (strpos(strtolower($useragent), "googlebot") !== false OR strpos(strtolower($useragent), "bingbot") !== false OR strpos(strtolower($useragent), "yahoo! slurp") !== false OR strpos(strtolower($useragent), "yandex") !== false) {
        ...
        if ($querybanned->num_rows <= 0) {
            $bannedcpage_url = $settings['bansystem_path'] . "/pages/banned-country.php";
            echo '<meta http-equiv="refresh" content="0;url=' . $bannedcpage_url . '" />';
            exit;
        }
    }
...
//Blocking Browser
$querybanned = $mysqli->query("SELECT * FROM `bansystem_bans-other` WHERE type='browser'");
while ($rowb = $querybanned->fetch_assoc()) {
    if (strpos(strtolower($browser), strtolower($rowb['value'])) !== false) {
        $blockedbpage_url = $settings['bansystem_path'] . "/pages/blocked-browser.php";
...
$querybanned = $mysqli->query("SELECT * FROM `bansystem_bans-other` WHERE type='os'");
while ($rowo = $querybanned->fetch_assoc()) {
    if (strpos(strtolower($os), strtolower($rowo['value'])) !== false) {
        $blockedopage_url = $settings['bansystem_path'] . "/pages/blocked-os.php";
...
$querybanned = $mysqli->query("SELECT * FROM `bansystem_bans-other` WHERE type='isp'");
while ($rowi = $querybanned->fetch_assoc()) {
    if (strpos(strtolower($isp_check), strtolower($rowi['value'])) !== false) {
        $blockedipage_url = $settings['bansystem_path'] . "/pages/blocked-isp.php";
...
//Blocking Referrer
$querybanned = $mysqli->query("SELECT * FROM `bansystem_bans-other` WHERE type='referrer'");
while ($rowr = $querybanned->fetch_assoc()) {
    if (strpos(strtolower(@$referer), strtolower($rowr['value'])) !== false) {
        $blockedrpage_url = $settings['bansystem_path'] . "/pages/blocked-referrer.php";
